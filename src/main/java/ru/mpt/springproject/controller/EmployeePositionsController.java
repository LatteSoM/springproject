...
public class EmployeePositionsController {
    ...
    public String editEmployeePosition(@PathVariable Long id, Model model) {
        EmployeePosition employeePosition = employeePositionService.getEmployeePositionById(id).orElseThrow(() -> new ResourceNotFoundException("EmployeePosition not exist with id :" + id));
        List<Employee> employees = employeeService.getAllEmployees();
        List<Position> positions = positionService.getAllPositions();
